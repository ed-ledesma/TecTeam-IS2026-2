...
def listar_usuarios():
    ...
    usuarios_con_rol = agregar_rol_a_usuarios(
        usuarios
    )
...
def listar_profesores():
    ...
    profesores = Profesor.query.join(
        Usuario
    ).order_by(
        Usuario.nombres.asc()
    ).all()
...
def crear_usuario():
    ...
    if request.method == "POST":
        ...
        errores = validar_datos_para_crear_usuario(
            datos
        )
        ...
        asignacion_rol = construir_asignacion_rol(
            datos
        )
...
def editar_usuario(id_usuario):
    ...
    rol_actual = encontrar_rol_de_usuario(
        usuario.id_usuario
    )
    ...
    if request.method == "POST":
        ...
        errores = validar_datos_para_editar_usuario(
            datos,
            usuario,
        )
...
@admin_bp.post(
    "/usuarios/<int:id_usuario>/activar"
)
@sesion_requerida
@rol_requerido(ROL_ADMINISTRADOR)
...
@admin_bp.post(
    "/usuarios/<int:id_usuario>/desactivar"
)
@sesion_requerida
@rol_requerido(ROL_ADMINISTRADOR)
...
def obtener_datos_formulario_usuario():
    ...
    return {
        ...
        "fecha_contratacion": request.form.get(
            "fecha_contratacion",
            "",
        ).strip(),

        "nivel_acceso": request.form.get(
            "nivel_acceso",
            "",
        ).strip(),

        "area_responsable": request.form.get(
            "area_responsable",
            "",
        ).strip(),
    }
...
def datos_desde_usuario(usuario, rol):
    ...
    if rol == ROL_PROFESOR:
        ...
        datos["fecha_contratacion"] = (
            profesor.fecha_contratacion.isoformat()
            if profesor else ""
        )
    ...
    if rol == ROL_ADMINISTRADOR:
        ...
        datos["nivel_acceso"] = (
            ...
            if administrador else "1"
        )

        datos["area_responsable"] = (
            administrador.area_responsable
...
def validar_datos_para_crear_usuario(datos):
    ...
    errores.extend(
        validar_datos_de_rol(datos)
    )

    return errores


def validar_datos_para_editar_usuario(
    datos,
    usuario,
):
    ...
    usuario_con_correo = Usuario.query.filter_by(
        correo=datos["correo"]
    ).first()

    if (
        usuario_con_correo
        and usuario_con_correo.id_usuario
        != usuario.id_usuario
    ):
        errores.append(
...
def validar_datos_de_rol(
    datos,
    id_usuario=None,
):

    if datos["rol"] == ROL_ALUMNO:
        return validar_datos_alumno(
            datos,
            id_usuario,
        )

    if datos["rol"] == ROL_PROFESOR:
        return validar_datos_profesor(
            datos,
            id_usuario,
        )

    if datos["rol"] == ROL_ADMINISTRADOR:
        return validar_datos_administrador(
            datos
        )
...
def validar_datos_alumno(
    ...
    alumno_con_matricula = Alumno.query.filter_by(
        matricula=datos["matricula"]
    ).first()

    if (
        alumno_con_matricula
        and alumno_con_matricula.id_usuario
        != id_usuario
    ):
        errores.append(
...
def validar_datos_profesor(
    ...
    if not datos["numero_empleado"]:
        ...
        errores.append(
            "El número de empleado es obligatorio para profesores."
        )
    ...
    if (
        profesor_con_numero
        and profesor_con_numero.id_usuario
        != id_usuario
    ):
        errores.append(
...
def validar_datos_administrador(datos):
    ...
    if not convertir_entero(
        datos["nivel_acceso"]
    ):
        errores.append(
            "El nivel de acceso debe ser un número entero positivo."
        )
...
def construir_usuario(datos):
    ...
    return Usuario(
        ...
        correo=datos["correo"],
        fecha_nacimiento=convertir_fecha(
            datos["fecha_nacimiento"]
        ),
        activo=True,
...
def actualizar_usuario(
    ...
    usuario.apellido_paterno = datos[
        "apellido_paterno"
    ]

    usuario.apellido_materno = datos[
        "apellido_materno"
    ]

    usuario.correo = datos["correo"]

    usuario.fecha_nacimiento = convertir_fecha(
        datos["fecha_nacimiento"]
    )
    ...
    if datos["password"]:
        ...
        usuario.ultimo_cambio_password = (
            datetime.utcnow()
        )
...
def construir_asignacion_rol(datos):
    ...
    if rol == ROL_PROFESOR:
        ...
        return Profesor(
            numero_empleado=datos[
                "numero_empleado"
            ],
            especialidad=datos[
                "especialidad"
            ] or None,
            fecha_contratacion=convertir_fecha(
                datos["fecha_contratacion"]
            ),
        )
    ...
    if rol == ROL_ADMINISTRADOR:
        ...
        return Administrador(
            nivel_acceso=convertir_entero(
                datos["nivel_acceso"]
            ),
            area_responsable=datos[
                "area_responsable"
            ] or None,
        )
...
def actualizar_asignacion_rol(
    ...
    if rol_actual != rol_nuevo:
        ...
        asignar_datos_especificos_de_rol(
            usuario.id_usuario,
            rol_nuevo,
            datos,
        )
...
def asignar_datos_especificos_de_rol(
    id_usuario,
    rol,
    datos,
):
    ...
    if rol == ROL_PROFESOR:
        ...
        profesor.numero_empleado = datos[
            "numero_empleado"
        ]

        profesor.especialidad = (
            datos["especialidad"] or None
        )

        profesor.fecha_contratacion = convertir_fecha(
            datos["fecha_contratacion"]
        )
    ...
    if rol == ROL_ADMINISTRADOR:
        ...
        administrador.nivel_acceso = convertir_entero(
            datos["nivel_acceso"]
        )

        administrador.area_responsable = (
            datos["area_responsable"] or None
        )
